...
def test_join_pdfs_form_valid(app):
    ...
    with app.test_request_context():
        ...
        try:
            ...
            files = MultiDict({"pdf_files": FileStorage(stream=open(temp_pdf.name, "rb"), filename="sample.pdf")})
...
def test_split_pdf_form_valid(app):
    ...
    with app.test_request_context():
        ...
        try:
            data = MultiDict({"captcha_answer": "1234"})
            files = MultiDict({"pdf_file": FileStorage(stream=open(temp_pdf.name, "rb"), filename="sample.pdf")})
